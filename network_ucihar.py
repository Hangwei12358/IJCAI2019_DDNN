...
class DDNN(nn.Module):
    def __init__(self, args):
        ...
        self.encoder = nn.Sequential(
            nn.Linear(self.n_feature * self.len_sw, 512),
            nn.ReLU(True),
            nn.Linear(512, 256),
            nn.ReLU(True),
            nn.Linear(256, 128),
            nn.ReLU(True),
            nn.Linear(128, self.d_AE))
        self.decoder = nn.Sequential(
            nn.Linear(self.d_AE, 128),
            nn.ReLU(True),
            nn.Linear(128, 256),
            nn.ReLU(True),
            nn.Linear(256, 512),
            nn.ReLU(True),
            nn.Linear(512, self.n_feature * self.len_sw),
            nn.Tanh())
        # rnn part
        ...
        self.fc1 = nn.Sequential(
            nn.Linear(in_features=(2 * self.n_lstm_hidden + self.d_AE + 448), out_features=1000),
            nn.ReLU()
        )
    ...
    def forward(self, x):
        ...
        out_rnn, _ = self.lstm(x.permute(0, 2, 1))
        out_rnn = out_rnn[:, -1, :]

        out_rnn_spatial, _ = self.lstm_spatial(x.view(x.shape[0], self.n_feature, -1))
